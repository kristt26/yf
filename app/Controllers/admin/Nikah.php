...
class Nikah extends BaseController
{
    ...
    public function index()
    {
        ...
        if (is_null($setItem)) {
            $url = base_url('manajemen_nikah') . '?item=' . enkrip("pengajuan");
            return redirect()->to($url);
        } else if (dekrip($setItem) == "pengajuan") {
            $list['title'] = "Pengajuan";
            $list['url'] = enkrip("pengajuan");
            $list['item'] = count($this->pendaftaran->getPengajuanAll("3"));
            return view('admin/nikah/pengajuan', $list);
        } else if (dekrip($setItem) == "proses") {
            $list['title'] = "Proses";
            $list['url'] = enkrip("proses");
            $list['add'] = enkrip("add");
            $list['item'] = count($this->pendaftaran->getPengajuanAll("3"));
            return view('admin/nikah/proses', $list);
        } else if (dekrip($setItem) == "selesai") {
            $list['title'] = "Selesai";
            $list['url'] = enkrip("selesai");
            $list['item'] = count($this->pendaftaran->getPengajuanAll("3"));
            return view('admin/nikah/selesai', $list);
        } else if (dekrip($setItem) == "add") {
            $list['title'] = "Add";
            $list['url'] = enkrip("proses");
            $list['item'] = count($this->pendaftaran->getPengajuanAll("3"));
            return view('admin/nikah/add', $list);
        }
    }

    public function getPengajuan()
    {
        $data = $this->pendaftaran->getPengajuanAll("3");
        logger('notice', $data);
    ...
    {
        $data = $this->pendaftaran->getProsesAll("3");
        logger('notice', $data);
        return $this->respond($data);
    }
    ...
    {
        $data = $this->pendaftaran->getSelesaiAll("3");
        logger('notice', $data);
        return $this->respond($data);
    }
