...
class Sidi extends BaseController
{
    ...
    public function index()
    {
        ...
        if (is_null($setItem)) {
            $url = base_url('layanan_sidi') . '?item=' . enkrip("pengajuan");
            return redirect()->to($url);
        } else if (dekrip($setItem) == "pengajuan") {
            $list['title'] = "Pengajuan";
            $list['url'] = enkrip("pengajuan");
            $list['add'] = enkrip("add");
            return view('jemaat/sidi/pengajuan', $list);
        } else if (dekrip($setItem) == "proses") {
            $list['title'] = "Proses";
            $list['url'] = enkrip("proses");
            return view('jemaat/sidi/proses', $list);
        } else if (dekrip($setItem) == "selesai") {
            $list['title'] = "Selesai";
            $list['url'] = enkrip("selesai");
            return view('jemaat/sidi/selesai', $list);
        } else if (dekrip($setItem) == "add") {
            $list['title'] = "Add";
            $list['url'] = enkrip("proses");
            return view('jemaat/sidi/add', $list);
        } else if (dekrip($setItem) == "edit") {
            $list['title'] = "Add";
            $list['url'] = enkrip("proses");
            return view('jemaat/sidi/edit', $list);
        }
    }
    public function getPengajuan()
    {
        $data = $this->pendaftaran->getPengajuanByUser("2");
        return $this->respond($data);
    }

    public function getProses()
    {
        $data = $this->pendaftaran->getProsesByUser("2");
        return $this->respond($data);
    }

    public function getSelesai()
    {
        $data = $this->pendaftaran->getSelesaiByUser("2");
        return $this->respond($data);
    }

    public function add()
    {
        return $this->respond($this->anggota->getBelumByKK('2'));
    }

    public function edit()
    {
        return $this->respond($this->pendaftaran->getById("2", $this->request->getGet('set')));
    }
