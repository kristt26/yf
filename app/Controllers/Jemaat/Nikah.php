...
class Nikah extends BaseController
{
    ...
    public function index()
    {
        ...
        if (is_null($setItem)) {
            $url = base_url('layanan_nikah') . '?item=' . enkrip("pengajuan");
            return redirect()->to($url);
        } else if (dekrip($setItem) == "pengajuan") {
            $list['title'] = "Pengajuan";
            $list['url'] = enkrip("pengajuan");
            $list['add'] = enkrip("add");
            return view('jemaat/nikah/pengajuan', $list);
        } else if (dekrip($setItem) == "proses") {
            $list['title'] = "Proses";
            $list['url'] = enkrip("proses");
            return view('jemaat/nikah/proses', $list);
        } else if (dekrip($setItem) == "selesai") {
            $list['title'] = "Selesai";
            $list['url'] = enkrip("selesai");
            return view('jemaat/nikah/selesai', $list);
        } else if (dekrip($setItem) == "add") {
            $list['title'] = "Add";
            $list['url'] = enkrip("proses");
            return view('jemaat/nikah/add', $list);
        } else if (dekrip($setItem) == "edit") {
            $list['title'] = "Add";
            $list['url'] = enkrip("proses");
            return view('jemaat/nikah/edit', $list);
        }
    }
    public function getPengajuan()
    {
        $data = $this->pendaftaran->getPengajuanByUser("3");
        return $this->respond($data);
    }

    public function getProses()
    {
        $data = $this->pendaftaran->getProsesByUser("3");
        return $this->respond($data);
    }

    public function getSelesai()
    {
        $data = $this->pendaftaran->getSelesaiByUser("3");
        return $this->respond($data);
    }

    public function add()
    {
        return $this->respond($this->anggota->getBelumByKK('3'));
    }

    public function edit()
    {
        return $this->respond($this->pendaftaran->getById("3", $this->request->getGet('set')));
    }
