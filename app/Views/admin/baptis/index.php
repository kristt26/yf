...
<div class="page-header">
    <h3 class="page-title"> Manajemen Baptis </h3>
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <?php endif; ?>
            <?php if ($title == 'Add') : ?>
                <li class="breadcrumb-item"><a href="<?= base_url("manajemen_baptis?item=") . $url ?>">Manajemen Baptis</a></li>
                <li class="breadcrumb-item active" aria-current="page">Daftarkan</li>
            <?php endif; ?>
        </ol>
    </nav>
</div>
<div class="row" ng-controller="manajemenBaptisController">
                    ...
                    <ul class="nav nav-pills mb-1" id="pills-tab" role="tablist">
                        <?php foreach ($lists as $key => $list) : ?>
                            <li class="nav-item" role="presentation">
                                <a href="<?= base_url("manajemen_baptis?item=") . $list['url'] ?>" class="nav-link <?= $url == $list['url'] ? 'active' : '' ?>" style="padding: 0.25rem 0.5rem;" data-bs-target="#<?= $list['url'] ?>" role="tab" aria-controls="<?= $list['url'] ?>" aria-selected="true"><?= $list['text'] ?>
